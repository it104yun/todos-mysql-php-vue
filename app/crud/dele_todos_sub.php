...
// 提取並驗證必要的欄位
$todos_id = $data['todos_id'] ?? null; // 也傳入 user_id 以確保只能刪除自己的資料
...
try {
    ...
    // --- 刪除相關聯的 sub_todos ---
    $sql_sub = "DELETE FROM sub_todos WHERE todos_id = :todos_id and item_id = :item_id";
    $stmt_sub = $pdo->prepare($sql_sub);
    $stmt_sub->bindParam(':todos_id', $todos_id, PDO::PARAM_INT);
